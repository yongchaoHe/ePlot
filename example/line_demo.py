...
def demo_1():
    ...
    data = [
        [
            ([0.1*item for item in range(101)], [math.sin(0.1*item-0.5)+1 for item in range(101)]),
            ([0.1*item for item in range(101)], [math.sin(0.1*item+0.5)+1 for item in range(101)]),
            ([0.1*item for item in range(101)], [math.sin(0.1*item-0.5)+2 for item in range(101)]),
            ([0.1*item for item in range(101)], [math.sin(0.1*item+0.5)+2 for item in range(101)])
        ]
    ]
    ...
    yaxis = axis(
        ...
        [str(item) for item in range(0, 6)]
    )
    xyaxis = [
        (xaxis, yaxis)
    ]
    ...
    myfig = Figure(1, 1, 4, 2.5, lwidth=1.5, alpha=0.8, fig_name="line_demo_1", ggplot=False)
    myfig.set_legends(legends, ncol_legend=2, loc_legend=100, adjust_legend=0.75)
    myfig.plot_line(data, xyaxis, hline, ls_plain=True, use_marker=False)
...
def demo_2():
    ...
    myfig = Figure(2, 2, 8, 5, lwidth=1.5, alpha=0.8, legend_size=-1, fig_name="line_demo_2", ggplot=False)
    myfig.set_legends(legends, ncol_legend=4, loc_legend=100, adjust_legend=0.9)
    myfig.plot_line(data, xyaxis, hline, ls_plain=False, use_marker=True)
...
def demo_3():
    ...
    myfig = Figure(2, 2, 8, 5, lwidth=1.5, alpha=0.8, legend_size=-1, fig_name="line_demo_3", ggplot=True)
    myfig.set_legends(legends, ncol_legend=1, loc_legend=100, adjust_legend=0.9)
    myfig.plot_line(data, xyaxis, hline, ls_plain=False, use_marker=False)
